...
if ( ! function_exists('sava_google_fonts') ) :
function sava_google_fonts() {

	if ( sava_theme_option( 'default_theme_fonts', 'default_fonts' ) == 'google_fonts' ) :

		$main_font 					= sava_theme_option( 'main_font', 'Roboto');
		$main_font_variants 		= sava_theme_option( 'main_font_variants', array('400'));

		$secondary_font 			= sava_theme_option( 'secondary_font', 'Roboto');		
		$secondary_font_variants 	= sava_theme_option( 'secondary_font_variants', array('400'));

		$google_fonts_subsets 		= sava_theme_option( 'google_fonts_subsets', '');
		...
		if ( !empty($font_family) ):
			...
			wp_enqueue_style( 'sava_google_fonts', $fonts_url, array(), null );

		endif;

	endif;
}            
add_action('wp_head', 'sava_google_fonts', 0);
endif;